...
import com.venturedive.blazetest.repository.TestRunDetailsRepository;
import com.venturedive.blazetest.service.criteria.TestRunDetailsCriteria;
...
class TestRunDetailsResourceIT {
    ...
    void getAllTestRunDetailsByTestRunIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, TestRuns.class).isEmpty()) {
            testRunDetailsRepository.saveAndFlush(testRunDetails);
            testRun = TestRunsResourceIT.createEntity(em);
        } else {
            testRun = TestUtil.findAll(em, TestRuns.class).get(0);
        }
        em.persist(testRun);
        em.flush();
    ...
    @Test
    @Transactional
    void getAllTestRunDetailsByTestrundetailattachmentsTestrundetailIsEqualToSomething() throws Exception {
        TestRunDetailAttachments testrundetailattachmentsTestrundetail;
        if (TestUtil.findAll(em, TestRunDetailAttachments.class).isEmpty()) {
            testRunDetailsRepository.saveAndFlush(testRunDetails);
            testrundetailattachmentsTestrundetail = TestRunDetailAttachmentsResourceIT.createEntity(em);
        } else {
            testrundetailattachmentsTestrundetail = TestUtil.findAll(em, TestRunDetailAttachments.class).get(0);
        }
        em.persist(testrundetailattachmentsTestrundetail);
        em.flush();
        testRunDetails.addTestrundetailattachmentsTestrundetail(testrundetailattachmentsTestrundetail);
        testRunDetailsRepository.saveAndFlush(testRunDetails);
        Long testrundetailattachmentsTestrundetailId = testrundetailattachmentsTestrundetail.getId();

        // Get all the testRunDetailsList where testrundetailattachmentsTestrundetail equals to testrundetailattachmentsTestrundetailId
        defaultTestRunDetailsShouldBeFound("testrundetailattachmentsTestrundetailId.equals=" + testrundetailattachmentsTestrundetailId);

        // Get all the testRunDetailsList where testrundetailattachmentsTestrundetail equals to (testrundetailattachmentsTestrundetailId + 1)
        defaultTestRunDetailsShouldNotBeFound(
            "testrundetailattachmentsTestrundetailId.equals=" + (testrundetailattachmentsTestrundetailId + 1)
        );
    }
