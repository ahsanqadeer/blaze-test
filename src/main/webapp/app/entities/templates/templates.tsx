...
export const Templates = () => {
  ...
  const totalItems = useAppSelector(state => state.templates.totalItems);
  ...
  return (
    <div>
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('templateName')}>
            ...
            <tbody>
              {templatesList.map((templates, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{templates.createdAt ? <TextFormat type="date" value={templates.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{templates.createdBy}</td>
                  <td>{templates.company ? <Link to={`/companies/${templates.company.id}`}>{templates.company.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/templates/${templates.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/templates/${templates.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={templatesList && templatesList.length > 0 ? '' : 'd-none'}>
